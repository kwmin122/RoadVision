...
5. **경고 히스테리시스**: 이탈 경고 ON/OFF에 다른 임계(예: ON>0.35, OFF<0.25) → 깜빡이는 경고 방지.
...
- 차량중심 `car_center_x = W/2` (카메라가 차량 중앙에 장착됐다 가정)
- 정규화 오프셋 `offset = (car_center_x − lane_center_x) / (lane_pixel_width/2)` → [-1,1]
- (선택) 미터 환산: 차선폭 3.7m 가정 → `offset_m = offset × 1.85m`
- 판정: `|offset| > 0.35` 면 해당 방향 "LANE DEPARTURE" 경고(빨강 배너 + 차선선 강조), 히스테리시스 적용.
...
- **C4 LDW 검증(합성 이동 테스트)**: Udacity 영상엔 실제 차선이탈 장면이 거의 없음 → **프레임/차선중앙을 인위적으로 좌우 평행이동시켜 경고 트리거를 검증**. 명시 케이스: 오프셋 0px(경고 OFF), **±80px(경계 부근)**, **±140px(경고 ON)** 에서 ON/OFF + 좌/우 방향이 의도대로 나오는지 표로 기록. 히스테리시스 ON>0.35/OFF<0.25 경계도 함께 확인.
